...
# define MENU_SET_VSYNC         5
# define MENU_SET_ANALOG        6
# define MENU_SET_AUTOFIRE      7
# define MENU_SET_DISPLAY_LR    8
...
# define MAX_MENU_SET_ITEM (MENU_SET_BACK + 1)
  ...
  static int ti99_auto_fire_period  = 0;
...
psp_display_screen_settings_menu(void)
{
  ...
  for (menu_id = 0; menu_id < MAX_MENU_SET_ITEM; menu_id++) {
    ...
    if (menu_id == MENU_SET_AUTOFIRE) {
      sprintf(buffer,"%d", ti99_auto_fire_period+1);
    ...
    if (menu_id == MENU_SET_RENDER) {
      ...
      if (ti99_render_mode == TI99_RENDER_FIT_HEIGHT) strcpy(buffer, "fit height");
...
{
  if (step > 0) {
    if (ti99_auto_fire_period < 19) ti99_auto_fire_period++;
  } else {
    if (ti99_auto_fire_period >  0) ti99_auto_fire_period--;
  }
}
...
{
  if (step > 0) {
    if (ti99_speed_limiter < 60) ti99_speed_limiter += 5;
    else                        ti99_speed_limiter = 0;
  } else {
    if (ti99_speed_limiter > 0) ti99_speed_limiter -= 5;
...
psp_settings_menu_init(void)
{
  ...
  psp_reverse_analog   = TI99.psp_reverse_analog;
  psp_display_lr       = TI99.psp_display_lr;
  ti99_vsync          = TI99.ti99_vsync;
  ti99_auto_fire_period = TI99.ti99_auto_fire_period;
}
...
void
psp_display_reboot_needed(void)
{
  psp_kbd_wait_no_button();
  psp_display_screen_settings_menu();
  psp_sdl_back2_print(270,  70, "TI99 Version Change", 
                     PSP_MENU_WARNING_COLOR);
  psp_sdl_back2_print(270,  80, "You need to Reboot",
                     PSP_MENU_WARNING_COLOR);
  psp_sdl_flip();
  psp_kbd_wait_button();
}
...
psp_settings_menu_validate(void)
{
  ...
  TI99.psp_reverse_analog  = psp_reverse_analog;
  TI99.psp_skip_max_frame  = ti99_skip_fps;
  TI99.psp_skip_cur_frame  = 0;

  TI99.ti99_view_fps        = ti99_view_fps;
  TI99.psp_display_lr      = psp_display_lr;
  TI99.ti99_auto_fire_period  = ti99_auto_fire_period;

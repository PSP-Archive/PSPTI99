...
psp_display_screen_kbd_menu(void)
{
  ...
  for (menu_id = 0; menu_id < MAX_MENU_KBD_ITEM; menu_id++) 
  {
        ...
        (menu_id <= MENU_KBD_JOY_RIGHT)) 
    {
      ...
      if (menu_kbd_selected == KBD_NORMAL_MAPPING  ) ti99_key = loc_kbd_mapping[kbd_id];
      else
      if (menu_kbd_selected == KBD_LTRIGGER_MAPPING) ti99_key = loc_kbd_mapping_L[kbd_id];
      else
      if (menu_kbd_selected == KBD_RTRIGGER_MAPPING) ti99_key = loc_kbd_mapping_R[kbd_id];

      if ((ti99_key >= 0) && (ti99_key < TI99K_MAX_KEY)) {
        strcpy(buffer, psp_ti99_key_info[ti99_key].name);
      } else 
      if (ti99_key == KBD_UNASSIGNED) {
        sprintf(buffer, "UNASSIGNED");
      } else
      if (ti99_key == KBD_LTRIGGER_MAPPING) {
        sprintf(buffer, "L MAPPING");
      } else
      if (ti99_key == KBD_RTRIGGER_MAPPING) {
        sprintf(buffer, "R MAPPING");
      } else {
        sprintf(buffer, "KEY %d", ti99_key);
      }
...
psp_keyboard_menu_mapping(int kbd_id, int step)
{
  ...
  if (ploc_kbd_mapping[kbd_id] <  -3) ploc_kbd_mapping[kbd_id] = TI99K_MAX_KEY-1;
  else
  if (ploc_kbd_mapping[kbd_id] >= TI99K_MAX_KEY) ploc_kbd_mapping[kbd_id] = -3;
}
...
# ifdef USE_PSP_IRKEYB
    ...
    if (psp_irkeyb != PSP_IRKEYB_EMPTY) {
      ...
      if (psp_irkeyb == 0x8) {
        ti99_key = TI99K_DELETE;
      } else
      if (psp_irkeyb == 0x9) {
        ti99_key = TI99K_TAB;
      } else
      if (psp_irkeyb == 0xd) {
        ti99_key = TI99K_RETURN;
      } else
      if (psp_irkeyb == 0x1b) {
        ti99_key = TI99K_ALT;
      } else
      ...
      if (psp_irkeyb == PSP_IRKEYB_RIGHT) {
        ti99_key = TI99K_RIGHT;
      } else {
        ti99_key = ti99_get_key_from_ascii(psp_irkeyb);
      }
      ...
      if (ti99_key != -1) {
            ...
            (cur_menu_id <= MENU_KBD_JOY_RIGHT)) 
        {
          ...
          if (menu_kbd_selected == -1) loc_kbd_mapping[kbd_id] = ti99_key;
          else
          if (menu_kbd_selected == KBD_LTRIGGER_MAPPING) loc_kbd_mapping_L[kbd_id] = ti99_key;
          else
          if (menu_kbd_selected == KBD_RTRIGGER_MAPPING) loc_kbd_mapping_R[kbd_id] = ti99_key;
        }
      }
    }
# endif

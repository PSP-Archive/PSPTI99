...
psp_sdl_save_thumb_png(SDL_Surface* my_surface, char* filename)
{
  ...
  for (y = 0; y < TI99_HEIGHT; y += 2) {
    for (x = 0; x < TI99_WIDTH; x += 2) {
      *dst_pixel++ = src_pixel[x];
    }
    src_pixel += TI99_WIDTH * 2;
  }
...
psp_sdl_save_screenshot(void)
{
  ...
  sprintf(TmpFileName,"%s/scr/screenshot_%d.png", TI99.ti99_home_dir, TI99.psp_screenshot_id++);
  if (TI99.psp_screenshot_id >= 10) TI99.psp_screenshot_id = 0;
...
# endif
  ...
  for (Index = 0; Index < TI99_MAX_SAVE_STATE; Index++) {
    TI99.ti99_save_state[Index].surface = 
